...
if (isset($_POST['transport'], $_POST['coords'], $_POST['distance'],  $_POST['graph'],  $_POST['date'])) {
    ...
    $excludeagency = $_POST['agency'];
    $excluderoute= $_POST['route'];


    if($excludeagency == ''){
        $banned_agencies = '';
    }else{
        $banned_agencies = '&bannedAgencies='. urlencode($excludeagency);
    }
    if($excluderoute == ''){
        $banned_routes = '';
    }else{
        $banned_routes = '&bannedRoutes='. urlencode($excluderoute);
    ...
    $results = build_iso($transport_type, $coords, $distance, $maxwalk, $graph, $date, $time, $url, $banned_agencies, $banned_routes);
...
function build_iso($transport_type, $coords, $distance, $maxwalk, $graph, $date, $time, $url, $banned_agencies, $banned_routes){
    ...
    $query = '/otp/routers/' .  $graph . '/isochrone?fromPlace=' . $coords .  '&mode=' . $transport_type . '&date=' . $date . '&time='. $time.'&maxWalkDistance='. $maxwalk. '&cutoffSec=' . 300 . '&cutoffSec=' . 600 . '&cutoffSec=' . 900 . '&cutoffSec=' . 1200  . '&cutoffSec=' . 1500 . '&cutoffSec=' . 1800 . '&cutoffSec=' . 2100 . '&cutoffSec=' . 2400 . '&cutoffSec=' . 2700 . '&cutoffSec=' . 3000 . '&cutoffSec=' . 3300 . '&cutoffSec=' . 3600 .$banned_agencies . $banned_routes ;
